<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE categoria set IDcategoria=?,cateDesc=?,grupoEdad=?,reproductiva=?,ordenCate=?,edadRelativa=?,sexoc=?,detalleOjo=? WHERE IDcategoria=? ";
            $q = $pdo->prepare($sql);
            $q->execute(array($IDcategoria,$cateDesc,$grupoEdad,$reproductiva,$ordenCate,$edadRelativa,$sexoc,$detalleOjo,$pk_IDcategoria));
            ...
            if ($q->rowCount()==0) {
                   ...
                   if ($arr[0] <> '00000') {
                       ...
                       $cateDesc = $data['cateDesc'];
                       $grupoEdad = $data['grupoEdad'];
                       $reproductiva = $data['reproductiva'];
                       $ordenCate = $data['ordenCate'];
                       $edadRelativa = $data['edadRelativa'];
                       $sexoc = $data['sexoc'];
                       $detalleOjo = $data['detalleOjo'];

                       $pk_IDcategoria = $IDcategoria;
...
<body onload="aVerVer('CRcategoria')">
                ...
                <form data-toggle="validator" id="CRcategoria" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
                    ...
                    <div class="form-group ">
                        <label for="IDcategoria">ID de la categor&iacute;a (c&oacute;digo)</label>
                            ...
                            <input name="pk_IDcategoria" type="hidden"  value="<?php echo !empty($pk_IDcategoria)?$pk_IDcategoria:'';?>"> <!-- pk, clave anterior -->
                    </div>
                ...
                <div class="col-sm-6">              
                    <div class="form-group ">
                        <label for="reproductiva">Es categor&iacute;a reproductiva?</label>
                            <select class="form-control input-sm" style="width: auto" id="reproductiva" required  data-error="Seleccionar un elemento de la lista" name="reproductiva">
