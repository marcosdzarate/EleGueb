...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
        ...
        $pQuery = "";
        if ($aCol == 'playa') {
           $pQuery = "SELECT nombre FROM playa WHERE IDplaya=? LIMIT 1";
            }
    ...
    if (edita() and $_SESSION['tipocen']<>'MENSUAL'   and editaCenso($aRow['fecha']) ) {
      $a = '<a class="btn btn-success" title="editar datos del grupo" onclick=ventanaM("grupo_editar.php?fecha='.urlencode($aRow['fecha']).'&libreta='.urlencode($aRow['libreta']).'&orden='.urlencode($aRow['orden']).'",this.title)>'.
	    ...
	    if ($crefer<>'ASOCIADOS') {
            $a .= '<a class="btn btn-info" title="composici&oacute;n del grupo" onclick=ventanaM("recuento_index.php?fecha='.urlencode($aRow['fecha']).'&libreta='.urlencode($aRow['libreta']).'&orden='.urlencode($aRow['orden']).'&xtrx='.urlencode($aRow['referencia']).'",this.title)>'.
               '<span class="glyphicon glyphicon-list"></span></a>';
		}
	  $row[] = $a;
    }
    else  {
      $row[] = '<a class="btn btn-default" title="vista de datos del grupo" onclick=ventanaM("grupo_editar.php?fecha='.urlencode($aRow['fecha']).'&libreta='.urlencode($aRow['libreta']).'&orden='.urlencode($aRow['orden']).'",this.title)>'.
