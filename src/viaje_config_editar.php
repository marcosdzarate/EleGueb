<?php
    ...
    $pk_fecha = null;  /* es la fecha_colocacion */
    if (isset($_GET["fecha"])) {
        $pk_fecha=$_GET["fecha"];
    ...
    if ( !empty($_POST)) {
		...
		$temporada=$pk_temporada;
		$tipoTempo=$pk_tipoTempo;
        ...
        if ($valid and isset($_POST['guardar'])) {
            ...
            $sql = "UPDATE viaje_config SET claveU=?, fecha_colocacion=?, profundidad=?,  profundidad_intervalo=?, temperatura=?, temperatura_intervalo=?, luz=?, luz_intervalo=?,  posicionamiento=?, angulo_pitch=?, angulo_roll=?, angulo_yaw=?, camara=?, argos_trans=?,  fin_viaje=?, estrategia=?, comentario=? WHERE claveU=? AND fecha_colocacion=?";
            $q = $pdo->prepare($sql);
			$q->execute(array($claveU,$fecha,$SQLprofundidad,$SQLprofundidad_intervalo, $SQLtemperatura,$SQLtemperatura_intervalo,$SQLluz, $SQLluz_intervalo,$SQLposicionamiento,$SQLangulo_pitch,$SQLangulo_roll,$SQLangulo_yaw, $SQLcamara,$SQLargos_trans,$fin_viaje,$SQLestrategia,$SQLcomentario,$pk_claveU,$pk_fecha));
		...
		if ($q->rowCount()==0) {
			   ...
			   if ($arr[0] <> '00000') {
				   ...
				   $data = $q->fetch(PDO::FETCH_ASSOC);
					...
					$profundidad_intervalo = $data['profundidad_intervalo'];
					$temperatura = $data['temperatura'];
					$temperatura_intervalo = $data['temperatura_intervalo'];
					$luz = $data['luz'];
					$luz_intervalo = $data['luz_intervalo'];
					$posicionamiento = $data['posicionamiento'];
					$angulo_pitch = $data['angulo_pitch'];
					$angulo_roll = $data['angulo_roll'];
					$angulo_yaw = $data['angulo_yaw'];
					$camara = $data['camara'];
					$argos_trans = $data['argos_trans'];
					$fin_viaje = $data['fin_viaje'];
					$estrategia = $data['estrategia'];
					$comentario = $data['comentario'];
					$fecha = $data['fecha_colocacion'];
...
<body onload="aVerVer('CRviaje_config')">
        ...
        <div class="panel panel-naranjino">
            ...
            <div class="panel-body">
                ...
                <form data-toggle="validator" id="CRviaje_config" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
                            <input name="pk_temporada" type="hidden"  value="<?php echo !empty($pk_temporada)?$pk_temporada:'';?>">
                            <input name="pk_tipoTempo" type="hidden"  value="<?php echo !empty($pk_tipoTempo)?$pk_tipoTempo:'';?>">							
								...
								<div class="col-sm-6" >
									<div class="form-group ">
										<label for="fin_viaje">Estado del viaje</label>
											<select class="form-control input-sm" style="width: auto" id="fin_viaje" required data-error="Seleccionar un elemento de la lista"   name="fin_viaje">
