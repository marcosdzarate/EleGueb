<?php
    ...
    if ( !empty($_POST)) {
        ...
        $marcaError = validar_marca ($marca,$valid,true);

        // validacion entre campos
        $eError = validarForm_marca ($claveU,$fecha,$marca,$comentario,$valid);
        ...
        if ($valid and isset($_POST['guardar'])) {
            ...
            $sql = "UPDATE marca set claveU=?,fecha=?,marca=?,comentario=? WHERE claveU=? AND fecha=? ";
            $q = $pdo->prepare($sql);
            $q->execute(array($claveU,$fecha,$marca,$SQLcomentario,$pk_claveU,$pk_fecha));
            ...
            $pdo = Database::connect();
            $sql = "SELECT * FROM marca where claveU=? AND fecha=? ";
            $q = $pdo->prepare($sql);
	...
	<span class="glyphicon glyphicon-question-sign"></span>
        ...
        <div class="panel panel-naranjino">
            <div class="panel-heading">
                <h3 class="panel-title">registro de marca</h3>
            </div>
            <div class="panel-body">

                <form data-toggle="validator" id="CRmarca" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
				...
				<div class="col-sm-2">						
                    ...
                    <div class="form-group ">
                        <label for="marca">Marca</label>
                            <input type="text" class="form-control input-sm" id="marca" style="width:250px" name="marca"  oninput="mayus(this);" maxlength="30" required pattern="<?php echo PATRON_marca?>" data-error="<?php echo PATRON_marca_men?>" value="<?php echo !empty($marca)?$marca:'';?>" >
                            <div class="help-block with-errors"></div>
                            <p id="JSErrormarca"></p>
					...
					<?php if (edita()) : ?>
                        <a class="btn btn-danger btn-sm"  onclick=ventanaMini("marca_borrar.php<?php echo $param;?>","")>eliminar</a><span style="display:inline-block; width: 20px;"></span>
					<?php endif ?>        
