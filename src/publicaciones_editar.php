<?php
	...
	/* TODOS PUEDEN MODIFICAR PUBLICACIONES /*
    ...
    if ( !empty($_POST)) {
		...
		$archiVista= limpia($_POST['archiVista']);
		...
		/* el archivo de la publicacion; archiVista viene sii hubo cambio en archivo */				
		$archivoError  = validar_publiArchivo ($archivo,$tipoArchivo,$valid,true);
		if (is_null($archivoError) and !empty($archiVista)) {
...
/*echo $ueb_Archi." ".$ArchiType," ".$check;*/
		...
		if ($valid) {
			...
			$tituH=MD5($titulo);			
			$sql = "UPDATE publicaciones set tipoPublicacion=?,anio=?,titulo=?,doi=?,autores=?,abstractYmas=?,archivo=?,tipoArchivo=?,tituloH=?  WHERE ID=? ";
			$q = $pdo->prepare($sql);
			$q->execute(array($tipoPublicacion,$anio,$titulo,$SQLdoi,$autores,$SQLabstractYmas,$archivo,$tipoArchivo,$tituH,$pk_ID));
            ...
            if ($q->rowCount()==0) {
                   ...
                   if ($arr[0] <> '00000') {
                       ...
                       $data = $q->fetch(PDO::FETCH_ASSOC);
						...
						$abstractYmas = $data['abstractYmas'];
						$doi = $data['doi'];
			...
			if ('files' in ar) {
				if (ar.files.length == 0) {
					...
					if (patt.test(arNombre)) {							
						...
						document.getElementById("archiVista").value=arNombre;
					}
					else{
						document.getElementById("archivo").value="";					
						document.getElementById("archiVista").value="";
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
				...
				<form data-toggle="validator" id="CRpublicacion" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"  method="post" enctype="multipart/form-data">				
					...
					<input type="hidden" id="archiVista" name="archiVista" >
						...
						<div class="col-sm-4">              
							...
							<div class="form-group ">
								<label for="autores">Autores</label>
									<textarea class="form-control input-sm" id="autores" name="autores" rows="2" maxlength="3000" required 
