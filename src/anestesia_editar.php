<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid and isset($_POST['guardar'])) {
            ...
            $sql = "UPDATE anestesia set claveU=?,fecha=?,hora=?,tipo=?,aplicacion=?,droga=?,droga_ml=?,partida=?,anotacion=? WHERE claveU=? AND fecha=? AND hora=? ";
            $q = $pdo->prepare($sql);
            $q->execute(array($claveU,$fecha,$hora,$tipo,$aplicacion,$droga,$SQLdroga_ml,$SQLpartida,$SQLanotacion,$pk_claveU,$pk_fecha,$pk_hora));
            ...
            if ($arr[0] <> '00000') {
                ...
                /*header('Location: anestesia_index.php');*/
                /*exit;*/
            ...
            $pdo = Database::connect();
            $sql = "SELECT * FROM anestesia where claveU=? AND fecha=? AND hora=? ";
            $q = $pdo->prepare($sql);
...
<script>
function aVerVer() {
	$('#CRanestesia').find(':input').attr('disabled', 'disabled'); 
}
</script>
<?php endif ?>
...
<body onload="setAplicacionDrogaPartida(document.getElementById('tipo')); <?php echo !edita()?'aVerVer()':''?>">
	...
	<span class="glyphicon glyphicon-question-sign"></span>
        ...
        <div class="panel panel-naranjino">
            <div class="panel-heading">
                <h3 class="panel-title">registro de anestesia</h3>
            </div>
				...
				<div class="row">
                    ...
                    <div class="form-group ">
                        <label for="tipo">Etapa</label>
                            <select class="form-control input-sm" style="width: auto" id="tipo" data-error="Seleccionar un elemento de la lista" oninput="setAplicacionDrogaPartida(this)" required  data-error="Seleccionar un elemento de la lista"
name="tipo">
				...
				<div class="row">
                    ...
                    <div class="form-group ">
                        <label for="partida">Partida</label>
                            <input type="text" class="form-control input-sm" id="partida" name="partida"  oninput="mayus(this);" pattern="<?php echo PATRON_partida;?>" maxlength="45"   data-error="<?php echo PATRON_partida_men;?>" value="<?php echo
!empty($partida)?$partida:'';?>" >
                            <div class="help-block with-errors"></div>
