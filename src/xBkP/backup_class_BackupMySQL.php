...
class BackupMySQL {
	...
	private $show = array( // (By default all)
		...
		'FOREIGN', // Generate SQL solo para FOREIGN KEYS (mrm-2018)
	);
	private $name = ""; // Backup file name (by default database name)
	private $folder = ""; // Backup target folder (by default temporary folder)

	/* columnas que no van a la salida de datos, tipo GEOMETRY (hay trigger en la tabla)
	   creando archivo .sql da problemas!! por eso la saco */
		
	private $keyQueNo = array();
	//——————————————————————————————————————————————
	// CONSTRUCTOR
	public function __construct($connection=array(), $tables=array(), $show=array(), $salida) {
	...
	public function setConnection($assoc) {
		...
		if (!isset($assoc['password'])) $assoc['password'] = "";
			if (!isset($assoc['port'])) $assoc['port'] = "";
	...
	public function set_keyQueNo($arre) {
		$this->keyQueNo = $arre;
	}
	//——————————————————————————————————————————————
	// PUBLIC METHODS
	...
	*/
	public function zip() {
		...
		if ($za->open($zip, ZipArchive::CREATE) !== true)
			throw new Exception ("No se puede abrir $zip");
	...
	*/
	...
	public function dameTablas() {  /* mrm 2018 */
	...
	private function initFile() {
		...
		$folder = empty($this->folder)? "" : rtrim($this->folder, '/').'/';
			...
			$this->path = "{$folder}{$name}.sql";   //_{$time}.sql";   //mrm 2018
	}
	...
	private function initDatabase() {
		...
		$charset = "utf8";
		$string = "mysql:host=$host;dbname=$database;charset=$charset;port=$port";
...
/*ECHO $table;*/
		...
		if ($this->show('FOREIGN')) {
			foreach ($this->dbtables as $table) $this->sqlCreateTableForeignKey($table);
	...
	*/
	...
	//	$result = $this->pdo->query("SHOW CREATE TABLE `$view`");
		$result = $this->pdo->query("SHOW CREATE VIEW `$view`");
		$array = $result->fetch(PDO::FETCH_NUM); // Table, Create Table
		$sql = $array[1];
		if (($pos = strpos($sql, 'CREATE')) !== false) $sql = "CREATE OR REPLACE ".substr($sql, $pos);
		$this->append("$sql;\n\n");
	}
	...
	private function sqlCreateTableForeignKey($table) {
		$result = $this->pdo->query("SHOW CREATE TABLE `$table`");
		$array = $result->fetch(PDO::FETCH_NUM); // Table, Create Table
		$sql = $array[1];
		$sql = substr($sql, 0, 12)." IF NOT EXISTS ".substr($sql, 13);
	...
	private function sqlTriggers() {
		$database = $this->connection['database'];
			...
			if(count($this->tables)==1){
				$nt=$this->tables[0];
				$sql="SHOW TRIGGERS FROM $database WHERE `Table`='$nt'";
				}
			else{
