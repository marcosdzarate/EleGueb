<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid and isset($_POST['guardar'])) {
            ...
            $sql = "UPDATE macho set claveU=?,fecha=?,status=?,estadoFisico=?,entornoAlAlfa=?,alAlfaCu=?,haremHembras=?,haremPups=?,comentario=? WHERE claveU=? AND fecha=? ";
            $q = $pdo->prepare($sql);
            $q->execute(array($claveU,$fecha,$status,$SQLestadoFisico,$SQLentornoAlAlfa,$SQLalAlfaCu,$SQLharemHembras,$SQLharemPups,$SQLcomentario,$pk_claveU,$pk_fecha));
            ...
            if ($arr[0] <> '00000') {
                ...
                /*header('Location: macho_index.php');*/
                /*exit;*/
            ...
            $pdo = Database::connect();
            $sql = "SELECT * FROM macho where claveU=? AND fecha=? ";
            $q = $pdo->prepare($sql);
            ...
            if ($q->rowCount()==0) {
                   ...
                   if ($arr[0] <> '00000') {
                       ...
                       $entornoAlAlfa = $data['entornoAlAlfa'];
                       $alAlfaCu = $data['alAlfaCu'];
                       $haremHembras = $data['haremHembras'];
                       $haremPups = $data['haremPups'];
...
<script>
function aVerVer() {
	$('#CRmacho').find(':input').attr('disabled', 'disabled'); 
}
</script>
<?php endif ?>
...
<body  onload='inicial(); <?php echo !edita()?"aVerVer()":"" ?>' >
	...
	<span class="glyphicon glyphicon-question-sign"></span>
        ...
        <div class="panel panel-naranjino">
            <div class="panel-heading">
                <h3 class="panel-title">registro de macho reproductivo</h3>
            </div>
                ...
                <form data-toggle="validator" id="CRmacho" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
						...
						<div class="col-sm-3">							
							<div class="form-group ">
								<label for="estadoFisico">Estado f&iacute;sico</label>
									<select class="form-control input-sm" style="width: auto" id="estadoFisico" data-error="Seleccionar un elemento de la lista"    data-error="Seleccionar un elemento de la lista" name="estadoFisico">
					...
					<div class="row" >
						<div class="col-sm-4">							
							...
							<?php if (edita()) : ?>
								<a class="btn btn-danger btn-sm"  onclick=ventanaMini("macho_borrar.php<?php echo $param;?>","")>eliminar</a><span style="display:inline-block; width: 20px;"></span>
							<?php endif ?>
