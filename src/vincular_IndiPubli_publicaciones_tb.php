<?php
	...
	$queIndi="";
	if (isset($_GET['queIndi'])) {
		$queIndi=$_GET['queIndi'];
	}
	
    $aColumns = array('ID','tipoPublicacion','anio','titulo','doi','autores','abstractYmas','archivo','tipoArchivo' );
   ...
   /*
	...
	if (empty($queIndi)){
		...
		/* solo las que tiene vinculo con queIndi en idpapers */
		if (empty($sWhere)) {
			$sWhere=" WHERE idpapers.claveU = '$queIndi' and publicaciones.ID=idpapers.IDpublicacion";
		}
		else{
			$sWhere .= " and idpapers.claveU = '$queIndi' and publicaciones.ID=idpapers.IDpublicacion";
		}
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable, idpapers
			$sWhere 
			$sOrder
			$sLimit
		";		
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
	...
	$rVincula="";
    if(!empty($queIndi)){
		$rVincula = '<a class="btn btn-warning" title="editar/quitar v&iacute;nculo" onclick=ventanaM("vincular_IndiPubli_editar.php?ID='.urlencode($aRow['ID']).'&claveU='.urlencode($queIndi).'",this.title)>'.
               '<span class="glyphicon glyphicon-edit"></span></a> ';
	}
	
	
    if (edita()) {
      $row[] = '<a class="btn btn-default" title="datos de la publicaci&oacute;n" target="_blank" onclick=ventanaM("publicaciones_ver.php?ID='.urlencode($aRow['ID']).'",this.title)>'.
               '<span class="glyphicon glyphicon-eye-open"></span></a> '.$rVincula;
    }
    else  {
    }
