<?php
	...
	if (isset($_GET["IDplaya"])) {
		$pk_IDplaya=$_GET["IDplaya"];
		$m = validar_IDplaya ($pk_IDplaya,$v,true);
	}
	else{
		if (!isset($_GET["IDplaya"]) and !isset($_POST["pk_IDplaya"]) ){
			$v=false;
		}
	}
	siErrorFuera($v);  
    ...
    if ( !empty($_POST)) {
        ...
        $pk_IDplaya=limpia($_POST['pk_IDplaya']);
        ...
        if ($valid) {
            ...
            $sql = "UPDATE playa set IDplaya=?,tipo=?,nombre=?,norteSur=?,geomTex=?,comentario=? WHERE IDplaya=? ";
            $q = $pdo->prepare($sql);
            $q->execute(array($IDplaya,$tipo,$nombre,$norteSur,$SQLgeomTex,$SQLcomentario,$pk_IDplaya));
            ...
            $pdo = Database::connect();
            $sql = "SELECT * FROM playa where IDplaya=? ";
            $q = $pdo->prepare($sql);
            $q->execute(array($pk_IDplaya));
            $arr = $q->errorInfo();
            if ($q->rowCount()==0) {
                    Database::disconnect();
                   ...
                   if ($arr[0] <> '00000') {
                       ...
                       $IDplaya = $data['IDplaya'];
                       $tipo = $data['tipo'];
                       $nombre = $data['nombre'];
                       $norteSur = $data['norteSur'];
                ...
                <div class="col-sm-6">              
                    ...
                    <div class="form-group ">
                        <label for="IDplaya">ID de playa (c&oacute;digo)</label>
                            <input type="text" class="form-control input-sm" style="width: 75px" id="IDplaya" name="IDplaya" <?php echo $ron?> oninput="mayus(this);" maxlength="5" required  pattern="<?php echo PATRON_IDplaya?>" data-error="<?php echo PATRON_IDplaya_men?>" value="<?php echo !empty($IDplaya)?$IDplaya:'';?>" >
                            ...
                            <input name="pk_IDplaya" type="hidden"  value="<?php echo !empty($pk_IDplaya)?$pk_IDplaya:'';?>"> <!-- pk, clave anterior -->
                    </div>
				</div>               
                <div class="col-sm-6">  
                    <div class="form-group ">
                        <label for="tipo">Tipo</label>
                            <select class="form-control input-sm" style="width: auto" id="tipo" data-error="Seleccionar un elemento de la lista" <?php echo $ron?> required  data-error="Seleccionar un elemento de la lista" name="tipo">
                                <option value="" <?php if ($tipo == "") {echo " selected";}?> ></option>
                                <option value="PUNTO" <?php if ($tipo == "PUNTO") {echo " selected";}?> <?php echo $dis?> >PUNTO</option>
                                <option value="TRAMO" <?php if ($tipo == "TRAMO") {echo " selected";}?> <?php echo $dis?> >TRAMO</option>
                                <option value="MENSUAL" <?php echo ($tipo=="MENSUAL")?" selected": " hidden ";?> >MENSUAL</option>
                                <option value="VIEJOTRAMO" <?php echo ($tipo=="VIEJOTRAMO")?" selected": " hidden ";?>  >VIEJOTRAMO</option>
                                <option value="VIEJOPUNTO" <?php echo ($tipo=="VIEJOPUNTO")?" selected": " hidden ";?> >VIEJOPUNTO</option>
                            </select>
                            <div class="help-block with-errors"></div>
                            <p id="JSErrortipo"></p>
                ...
                <div class="col-sm-9">  
                    <div class="form-group ">
                        <label for="nombre">Nombre</label>
                            <input type="text" class="form-control input-sm" id="nombre" name="nombre"  oninput="mayus(this);" <?php echo $ron?> pattern="<?php echo PATRON_nombrePlaya?>" data-error="<?php echo PATRON_nombrePlaya_men?>" maxlength="60" required  value="<?php echo !empty($nombre)?$nombre:'';?>" >
                            <div class="help-block with-errors"></div>
                ...
                <div class="col-sm-3">              
                    ...
                    <div class="form-group ">
                        <label for="norteSur">Orden Norte-Sur</label>
                            <input type="text" class="form-control input-sm" style="width:105px" id="norteSur" name="norteSur" onblur="tdecim(this, 2);" <?php echo $ron?> data-pdecimal
