<?php
    ...
    // validacion entre campos
        ...
        if ($valid) {
            ...
            $sql = "INSERT INTO sector (fecha,libreta,horaInicio,horaFin,geomTex,zonaRecorrida,direccionRecorrida,marea,clima,comentario,latiInicio,longInicio,latiFin,longFin) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $q = $pdo->prepare($sql);
            $q->execute(array($fecha,$libreta,$SQLhoraInicio,$SQLhoraFin,$SQLgeomTex,$SQLzonaRecorrida,$SQLdireccionRecorrida,$SQLmarea,$SQLclima,$SQLcomentario,$SQLlatiInicio,$SQLlongInicio,$SQLlatiFin,$SQLlongFin));
			...
			<div class="col-sm-3">
                    <div class="form-group ">
                        <label for="horaFin">Hora de finalizaci&oacute;n</label>
                            <input type="text" class="form-control input-sm" style="width:120px" id="horaFin" name="horaFin" placeholder="HH:mm:ss"  pattern="<?php echo PATRON_hora;?>" maxlength="8"  onchange="aSegundos(this)"  data-error="<?php echo PATRON_hora_men;?>" value="<?php echo !empty($horaFin)?$horaFin:'';?>" >
                            <div class="help-block with-errors"></div>
                        ...
                        <a class="btn btn-default btn-sm" onclick=parent.cierroM("vw_censo_sector_index.php?fechaTotal=<?php echo $_SESSION['fecTot'];?>")>atr&aacute;s</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 						...
 						<br>
                          ...
                          <?php if (empty($eError) and !empty($valid) and $valid ): ?>
                            <span class="alert alert-success"><h5>registro de sector agregado</h5></span>
                          <?php endif;?>
