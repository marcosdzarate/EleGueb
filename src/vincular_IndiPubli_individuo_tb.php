<?php
     ...
     */
    $aColumns = array('claveU','sexo','nuestro','tags','marcas','rango','cantTempo','titempos','tteta' );
	 ...
	 * elimino en BINARY del WHERE (no es casesensitive!)
     */
    $sWhere = "";
    if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
    {
		...
		{   /* lista de nombres en marcas */
		    $arr = explode(",",$_GET['sSearch']);
			$sIN= '("'.implode('","',$arr).'") ';
			$sWhere .= " marcas IN ".$sIN;
		}
   ...
   /*
     ...
     */
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS "."vw_seleccion_indi.".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere
			$sOrder
			$sLimit
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
	  ...
	  $ctm = $aRow['claveU'].' -- '.$aRow['tags'].' -- '.$aRow['marcas'];
      $row[] = '<a class="btn btn-default" target="_blank" title="ficha del individuo" href="tr_resumen_individuo.php?claveU='.urlencode($aRow['claveU']).'">'.
               '<span class="glyphicon glyphicon-option-horizontal"></span></a> '.
			   '<button class="btn btn-info" title="publicaciones vinculadas" onclick="vinculadas('."'".urlencode($aRow['claveU'])."','".$ctm."')".'">'.
