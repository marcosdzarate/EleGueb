<?php
     ...
     */
	/* vw_seleccion_indi (vista) tiene info de la presencia en cada tabla de un individuo */
	/* hasta cantTempo (incluido) las columnas son visibles en la interfaz, el resto no */
	/* pero sirven para la búsqueda que hace el usuario */
	/* si agregaste un columna para búsqueda (en la vista vw_seleccion_indi) colocar el nombre al final del arreglo */
	
    $aColumns = array('claveU','sexo','nuestro','muerto','tags','marcas','rango','cantTempo',
    ...
    if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            {
                $sWhere .= $aColumns[$i]." LIKE BINARY '%".mysqli_real_escape_string($gaSql['link'], $_GET['sSearch'] )."%' OR ";			
            }
        }
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
        if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
        {
			...
			if( $_GET['sSearch_'.$i] <> "%-") {
               $sWhere .= $aColumns[$i]." LIKE BINARY '%".mysqli_real_escape_string($gaSql['link'], $_GET['sSearch_'.$i])."%' ";
			}
			else{
			   /* pide NULL */
...
while ( $aRow = mysqli_fetch_array( $rResult ) )
{
    ...
    {
        $aCol = $aColumns[$i];
		if ($i>=11 and $aRow[ $aCol ] <> "" ) {
			$row[] = htmlspecialchars (substr($aRow[$aCol],0,3));//htmlspecialchars ("X");
		}
		else {
			$row[] = htmlspecialchars (str_replace(","," ",$aRow[ $aCol ]));
		}

    }

      $row[] = '<a class="btn btn-default" title="ficha del individuo" href="tr_resumen_individuo.php?claveU='.urlencode($aRow['claveU']).'">'.
