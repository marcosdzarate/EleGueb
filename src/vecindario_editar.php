<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            if(($tipo=='PARCIAL' OR $tipo=='MUDA') and $pk_IDesquema<>$IDesquema ){
			...
			if ($valid){            // actualiza
				$sql = "UPDATE vecindario set IDesquema=?,IDvecindario=?,tipo=?,NSdesde=?,NShasta=? WHERE IDesquema=? AND IDvecindario=? ";
				$q = $pdo->prepare($sql);
				$q->execute(array($IDesquema,$IDvecindario,$tipo,$NSdesde,$NShasta,$pk_IDesquema,$pk_IDvecindario));
            ...
            $pdo = Database::connect();
            $sql = "SELECT * FROM vecindario where IDesquema=? AND IDvecindario=? ";
            $q = $pdo->prepare($sql);
            ...
            if ($q->rowCount()==0) {
                   ...
                   if ($arr[0] <> '00000') {
                       ...
                       $IDvecindario = $data['IDvecindario'];
                       $tipo = $data['tipo'];
                       $NSdesde = $data['NSdesde'];
                       $NShasta = $data['NShasta'];

                       $pk_IDesquema = $IDesquema;
                       $pk_IDvecindario = $IDvecindario;
...
<body onload="aVerVer('CRvecindario')">
                ...
                <form data-toggle="validator" id="CRvecindario" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
                    ...
                    <div class="form-group ">
                        <label for="IDesquema">IDesquema</label>
                            <input type="number" class="form-control input-sm" style="width:90px" id="IDesquema" name="IDesquema" 
							data-pentero 
							data-dmin="<?php echo CONST_IDesquema_min;?>" data-dmax="<?php echo CONST_IDesquema_max;?>" required  
							data-error="<?php echo CONST_IDesquema_men;?>" 
							value="<?php echo !empty($IDesquema)?$IDesquema:'';?>" >
                            ...
                            <input name="pk_IDesquema" type="hidden"  value="<?php echo !empty($pk_IDesquema)?$pk_IDesquema:'';?>"> <!-- pk, clave anterior -->
                    </div>

                    <div class="form-group ">
                        <label for="IDvecindario">IDvecindario</label>
                            <textarea class="form-control input-sm" id="IDvecindario" name="IDvecindario" rows="1" maxlength="100" oninput="mayus(this);"  required  data-patron="<?php echo PATRON_IDvecin?>" data-error="<?php echo PATRON_IDvecin_men?>" ><?php echo !empty($IDvecindario)?$IDvecindario:'';?></textarea>
                            <div class="help-block with-errors"></div>
                            ...
                            <input name="pk_IDvecindario" type="hidden"  value="<?php echo !empty($pk_IDvecindario)?$pk_IDvecindario:'';?>"> <!-- pk, clave anterior -->
                    </div>
        ...
        ?>
                    ...
                    <div class="form-group ">
                        <label for="NShasta">&nbsp;&nbsp;&nbsp;&nbsp;</label>					
							...
							data-pdecimal 
							data-dmin="<?php echo CONST_norteSur_min;?>" data-dmax="<?php echo CONST_norteSur_max;?>"  readonly required  
							data-error="<?php echo CONST_norteSur_men;?>"  value="<?php echo !empty($NShasta)?$NShasta:'';?>" >
                            <div class="help-block with-errors"></div>
